...
1. 修改 MinIO 默认密码。
2. 通过 Nginx/Caddy 为 `LIU_MINIO_PUBLIC_URL` 配置 HTTPS 反代（MinIO API 端口 9000），并确保 **PUT 请求**能转发到 MinIO（不仅是 GET）。MinIO 容器需设置 `MINIO_SERVER_URL` 与公网地址一致（Compose 已默认注入）。
3. 若前端部署在 Cloudflare Pages，需重新构建前端以包含 MinIO 上传逻辑（`upload-via-minio`）。

### 上传报 `object not found`（E4004）

表示浏览器 PUT 之后，后端在 MinIO 里找不到对象。常见原因：

1. **`LIU_MINIO_PUBLIC_URL` 不正确** — 必须是浏览器能 PUT 到的地址，不能填 `http://minio:9000` 或 `localhost`（远程用户访问不到）。
2. **Nginx 反代未转发 PUT** — 检查 `proxy_pass` 是否允许 PUT/POST，且路径包含 `/liubai/` bucket 前缀。
3. **MinIO 未设置公网 URL** — 确认 `minio` 容器有 `MINIO_SERVER_URL=https://你的-storage-域名`。

排查：

```bash
# 查看 runtime 里的 MinIO 公网地址
docker exec liubai-runtime printenv LIU_MINIO_PUBLIC_URL

# 手动测试 MinIO 是否可写（在服务器上）
docker exec liubai-minio mc alias set local http://localhost:9000 minioadmin minioadmin
docker exec liubai-minio mc ls local/liubai
```
